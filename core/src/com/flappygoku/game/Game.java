...
public class Game extends ApplicationAdapter {
	...
	private void verificarEstadoJogo() {
		...
		if (estadoJogo == 0) {
			...
			// Aplicando gravidade no Bakugou
	...
	private void detectarColisao(){
		...
		shapeRenderer.circle(30 + gokus[1].getWidth() / 2,posicaoInicialVerticalGoku + gokus[1].getHeight()/2,gokus[1].getWidth()/2);
		//cima
	...
	public void validarPontos(){
		if (posicaoTorreHorizontal < 30 - gokus[0].getWidth()) {
			if (!passouTorre){
				pontos++;
				passouTorre = true;
				somPontuaçao.play();
	...
	private void inicializarObjetos(){
		...
		posicaoInicialVerticalGoku = alturaDispositivo /2;
		posicaoTorreHorizontal = larguraDispositivo;
		espacoEntreTorre =  500;
		posicaoMoeda = larguraDispositivo/2;

		//configurações dos textos
		textoPontuaçao = new BitmapFont();
		textoPontuaçao.setColor(Color.GOLD);
		textoPontuaçao.getData().setScale(6);
		textoReiniciar = new BitmapFont();
		textoReiniciar.setColor(Color.GREEN);
		textoReiniciar.getData().setScale(3);

		textoMelhorPontuaçao = new BitmapFont();
		textoMelhorPontuaçao.setColor(Color.RED);
